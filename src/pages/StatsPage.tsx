    ...
    <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-baseline gap-4 flex-wrap">
              {stats.runtimeDays > 0 && (
                <div className="flex items-baseline gap-1.5">
                  <span className="text-4xl sm:text-5xl font-black text-ink-50 tracking-tight">{stats.runtimeDays}</span>
                  <span className="text-sm sm:text-base font-bold text-gold-400 uppercase">Gün</span>
                </div>
              )}
              <div className="flex items-baseline gap-1.5">
                <span className="text-4xl sm:text-5xl font-black text-ink-50 tracking-tight">{stats.runtimeHours}</span>
                <span className="text-sm sm:text-base font-bold text-azure-400 uppercase">Saat</span>
              </div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-4xl sm:text-5xl font-black text-ink-50 tracking-tight">{stats.runtimeMins}</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        <div className="bg-ink-900/70 border border-ink-700/50 rounded-2xl p-4 shadow-lg flex flex-col justify-between">
          ...
          <div className="text-[11px] text-ink-400 mt-1">Film Ort: <strong className="text-gold-400">{stats.avgMovie.toFixed(1)}</strong></div>
        </div>
        ...
        <div className="bg-ink-900/70 border border-ink-700/50 rounded-2xl p-4 shadow-lg flex flex-col justify-between">
          ...
          <div className="text-[11px] text-ink-400 mt-1">Dizi Ort: <strong className="text-azure-400">{stats.avgSeries.toFixed(1)}</strong> ({stats.uniqueSeriesCount} Dizi)</div>
        </div>
        ...
        <div className="bg-ink-900/70 border border-ink-700/50 rounded-2xl p-4 shadow-lg flex flex-col justify-between">
          ...
          <div className="text-3xl font-black text-ink-50 mt-2">{data.achievements.reduce((s, a) => s + a.unlockedTiers.length, 0)}</div>
          <div className="text-[11px] text-ink-400 mt-1">Toplam: <strong className="text-emerald-400">{(data.totalXp || 0).toLocaleString()} XP</strong></div>
        </div>
      </div>
      ...
      <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
        <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
          <div>
            <h2 className="text-lg font-bold text-ink-100 flex items-center gap-2"><Tag size={20} className="text-gold-400" /> Değerlendirme Başlıkları İstatistikleri</h2>
            <p className="text-xs text-ink-400 mt-0.5">Puanlama sırasında seçtiğin başlıkların kullanım sıklığı ve o başlıklara verdiğin ortalama puanlar</p>
          </div>
          {stats.totalTaggedItems > 0 && (
            <span className="text-xs font-bold bg-gold-500/15 text-gold-300 border border-gold-500/30 px-3 py-1 rounded-full">{stats.totalTaggedItems} Yapımda Başlık Seçildi</span>
          )}
        </div>
        {stats.reviewTagStats.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {stats.reviewTagStats.map((item) => {
              ...
              return (
                <div key={item.tag} className="bg-ink-950/75 border border-ink-800/80 hover:border-gold-500/40 rounded-xl p-3.5 flex flex-col justify-between gap-2.5 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl space-y-3.5">
          <div>
            <h2 className="text-lg font-bold text-ink-100 flex items-center gap-2"><Trophy size={20} className="text-gold-400" /> Sinevia Rekorlar Kitabı</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <div className="bg-ink-950/70 border border-ink-800/80 rounded-xl p-3.5">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-gold-400 uppercase"><Flame size={13} /> En Yoğun Maraton Günü</div>
              <div className="text-lg font-black text-ink-50 mt-1">{stats.busiestDay.count > 0 ? `${stats.busiestDay.count} Yapım / Gün` : 'Henüz Yok'}</div>
              <div className="text-[11px] text-ink-400 mt-0.5 truncate">{stats.busiestDay.date}</div>
            </div>
            <div className="bg-ink-950/70 border border-ink-800/80 rounded-xl p-3.5">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-orange-400 uppercase"><Zap size={13} /> En Uzun Seri Rekoru</div>
              <div className="text-lg font-black text-ink-50 mt-1">{stats.maxStreakEver} Gün Aralıksız</div>
              <div className="text-[11px] text-ink-400 mt-0.5">Şu anki aktif seri: {data.dailyStreak} gün</div>
            </div>
            <div className="bg-ink-950/70 border border-ink-800/80 rounded-xl p-3.5">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-azure-400 uppercase"><Clock size={13} /> İzlenen En Uzun Film</div>
              <div className="text-sm font-black text-ink-50 mt-1 truncate">{stats.longestMovie ? stats.longestMovie.title : 'Veri Yok'}</div>
              <div className="text-[11px] text-ink-400 mt-0.5">{stats.longestMovie ? `${stats.longestMovie.runtime} Dakika` : '-'}</div>
            </div>
            <div className="bg-ink-950/70 border border-ink-800/80 rounded-xl p-3.5">
              <div className="flex items-center gap-1.5 text-[11px] font-bold text-violet-400 uppercase"><Tv size={13} /> En Çok Bölüm İzlenen Dizi</div>
              <div className="text-sm font-black text-ink-50 mt-1 truncate">{stats.mostWatchedSeries ? stats.mostWatchedSeries.title : 'Veri Yok'}</div>
              <div className="text-[11px] text-ink-400 mt-0.5">{stats.mostWatchedSeries ? `Tam ${stats.mostWatchedSeries.count} Bölüm İzlendi` : '-'}</div>
            </div>
          </div>
          <div className="bg-ink-950/50 border border-ink-800/60 rounded-xl p-3 flex items-center justify-between">
            <div className="flex items-center gap-2.5"><StickyNote size={16} className="text-emerald-400" /><span className="text-xs font-bold text-ink-200">Eleştirmen Kalemi & Günlük Notların</span></div>
            <span className="text-xs font-black text-emerald-400">{stats.notesCount} İnceleme ({stats.totalNoteWords} Kelime)</span>
          </div>
        </div>
        ...
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-bold text-ink-100 flex items-center gap-2"><Globe size={20} className="text-azure-400" /> Dünya Sineması Pasaportu</h2>
            <p className="text-xs text-ink-400 mt-0.5 mb-4">İzlediğin yapımların orijinal dillerine ve ülke sinemalarına göre dağılımı</p>
            {stats.topLanguages.length === 0 ? (
              <div className="text-center py-8 bg-ink-950/40 rounded-xl border border-ink-800/60">
                <p className="text-xs text-ink-400 px-4">Dil/Ülke verisi bulunamadı. Filmler veya Diziler sayfasındaki <strong>"Eksikleri Bul"</strong> butonuna tıklayarak TMDB verilerini senkronize edebilirsin.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  <div key={lang.code} className="bg-ink-950/70 border border-ink-800/80 rounded-xl p-3 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px] text-ink-400 font-semibold">{lang.count} Yapım İzlendi</div>
                      </div>
                    </div>
      ...
      <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h2 className="text-lg font-bold text-ink-100 flex items-center gap-2"><Crown size={20} className="text-gold-400" /> Yıldızlar Geçidi (Kişiler & Stüdyolar)</h2>
            <p className="text-xs text-ink-400 mt-0.5">İzlediğin yapımların künyelerine göre en çok tercih ettiğin isimler ve ortalama puanların</p>
          </div>
          <div className="flex bg-ink-950 rounded-xl p-1 border border-ink-800 w-fit">
            <button type="button" onClick={() => setPeopleTab('directors')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${peopleTab === 'directors' ? 'bg-gold-500 text-ink-950 shadow-sm' : 'text-ink-400 hover:text-ink-200'}`}><User size={13} /> Yönetmenler</button>
            <button type="button" onClick={() => setPeopleTab('cast')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${peopleTab === 'cast' ? 'bg-gold-500 text-ink-950 shadow-sm' : 'text-ink-400 hover:text-ink-200'}`}><Users size={13} /> Oyuncular</button>
            <button type="button" onClick={() => setPeopleTab('studios')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${peopleTab === 'studios' ? 'bg-gold-500 text-ink-950 shadow-sm' : 'text-ink-400 hover:text-ink-200'}`}><Building2 size={13} /> Stüdyolar</button>
          </div>
        </div>
        ...
        {(() => {
          ...
          if (activeList.length === 0) {
            return (
              <div className="text-center py-8 bg-ink-950/40 rounded-xl border border-ink-800/60">
                <p className="text-xs text-ink-400">Henüz künye verisi bulunamadı. Filmler veya Diziler sayfasındaki <strong>"Eksikleri Bul"</strong> butonuna basarak TMDB yönetmen/oyuncu verilerini çekebilirsin.</p>
              </div>
            );
          }
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {activeList.map((item, idx) => (
                <div key={item.name} className="bg-ink-950/70 border border-ink-800/80 hover:border-gold-500/40 rounded-xl p-3.5 flex items-center justify-between gap-3 transition-all">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs flex-shrink-0 ${idx === 0 ? 'bg-gold-500 text-ink-950 shadow-md' : idx === 1 ? 'bg-slate-300 text-slate-900' : idx === 2 ? 'bg-amber-700 text-white' : 'bg-ink-800 text-ink-300 border border-ink-700'}`}>#{idx + 1}</div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-ink-100 truncate">{item.name}</div>
                      <div className="text-[11px] text-ink-400 font-medium">{item.count} Yapım İzlendi</div>
                    </div>
                  </div>
                  {item.avg > 0 && <span className={`text-xs px-2.5 py-1 rounded-lg font-black flex-shrink-0 ${ratingBgClass(item.avg)}`}>{item.avg.toFixed(1)}</span>}
                </div>
              ))}
            </div>
          );
      ...
      {stats.topRated.length > 0 && (
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
          <div className="mb-4">
            <h2 className="text-lg font-bold text-ink-100 flex items-center gap-2"><Star size={20} className="text-gold-400 fill-current" /> Şeref Kürsüsü (En Yüksek Puanlı Yapımların)</h2>
            <p className="text-xs text-ink-400 mt-0.5">Detaylarını ve inceleme notlarını görmek için posterlere tıklayabilirsin</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3.5">
            {stats.topRated.map((h, i) => {
              ...
              return (
                <button key={h.id} type="button" onClick={() => openHistoryItemDetail(h)} className="group relative flex flex-col text-left bg-ink-950/70 border border-ink-800 hover:border-gold-500/50 rounded-2xl overflow-hidden shadow-lg transition-all hover:-translate-y-1 cursor-pointer">
                  <div className="aspect-[2/3] w-full bg-ink-900 relative overflow-hidden">
                    {poster ? <img src={poster} alt={h.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center text-ink-600">{isMovie ? <Film size={28} /> : <Tv size={28} />}</div>}
                    <div className="absolute top-2 left-2 w-6 h-6 rounded-lg bg-black/80 backdrop-blur-md border border-white/20 flex items-center justify-center text-[11px] font-black text-gold-400">#{i + 1}</div>
                    {h.rating !== null && <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-lg text-xs font-black shadow-md ${ratingBgClass(h.rating)}`}>{h.rating}</div>}
                  ...
                  <div className="p-2.5">
                    <div className="text-xs font-bold text-ink-100 truncate group-hover:text-gold-400 transition-colors">{h.title}</div>
      ...
      {(stats.criteriaAverages.length > 0 || stats.decades.length > 0) && (
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
            <h2 className="text-lg font-bold text-ink-100 mb-1 flex items-center gap-2"><SlidersHorizontal size={20} className="text-azure-400" /> Detaylı Kriter Karnesi</h2>
            <p className="text-xs text-ink-400 mb-4">Alt kriterlerde verdiğin ortalama puanların dağılımı</p>
                  ...
                  return (
                    <div key={crit.id} className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-ink-100 flex items-center gap-1.5">{crit.name} <span className="text-[10px] text-ink-500 font-normal">({crit.count} yapım)</span></span>
                        <span className={`px-2 py-0.5 rounded font-black text-[11px] ${ratingBgClass(crit.avg)}`}>{crit.avg.toFixed(1)} / 10</span>
                      </div>
          ...
          <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
            <h2 className="text-lg font-bold text-ink-100 mb-1 flex items-center gap-2"><Compass size={20} className="text-gold-400" /> Dönemlere Göre Tercihlerin</h2>
                ...
                {stats.decades.map((d) => {
                  ...
                  return (
                    <div key={d.decade} className="flex items-center gap-3">
                      <div className="w-16 text-xs font-black text-ink-200 flex-shrink-0">{d.decade}</div>
      ...
      <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
        <h2 className="text-lg font-semibold text-ink-100 mb-4 flex items-center gap-2"><Layers size={20} className="text-teal-400" /> En Sevdiğin Türler</h2>
              ...
              return (
                <div key={genre} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-7 bg-ink-800/60 rounded-lg overflow-hidden relative group">
                    <div className={`h-full bg-gradient-to-r ${colorClass} rounded-lg flex items-center justify-end pr-2.5 transition-all duration-700 ease-out`} style={{ width: `${Math.max(8, (val.count / maxGenreCount) * 100)}%` }}>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
          <h3 className="text-sm font-semibold text-ink-300 mb-3 flex items-center gap-2"><Film size={16} className="text-teal-400" /> Film Türleri</h3>
            ...
            return (
              <div key={genre} className="flex items-center justify-between py-2 text-sm border-b border-ink-800/50 last:border-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {avg > 0 && <span className={`text-xs w-8 text-center py-0.5 rounded font-bold ${ratingBgClass(avg)}`}>{avg.toFixed(1)}</span>}
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
          <h3 className="text-sm font-semibold text-ink-300 mb-3 flex items-center gap-2"><Tv size={16} className="text-cyan-400" /> Dizi Türleri</h3>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <h2 className="text-base font-semibold text-ink-100 mb-4 flex items-center gap-2"><Calendar size={18} className="text-cyan-400" /> Aylık İzleme</h2>
        ...
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <h2 className="text-base font-semibold text-ink-100 mb-4 flex items-center gap-2"><Calendar size={18} className="text-violet-400" /> Haftalık Yoğunluk</h2>
        ...
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
          <h2 className="text-base font-semibold text-ink-100 mb-4 flex items-center gap-2"><TrendingUp size={18} className="text-amber-400" /> Puan Dağılımı</h2>
      ...
      {stats.collectionStats.length > 0 && (
        <div className="bg-ink-900/60 backdrop-blur-sm border border-ink-700/50 rounded-2xl p-5 shadow-xl">
          <h2 className="text-lg font-semibold text-ink-100 mb-4 flex items-center gap-2"><Layers size={20} className="text-cyan-400" /> Koleksiyon İlerlemesi</h2>
